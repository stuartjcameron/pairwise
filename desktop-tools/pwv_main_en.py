...
class JudgePanel(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.play_left.clicked.connect(self.play_left)
        self.ui.play_right.clicked.connect(self.play_right)
    ...
    def show_time_warning(self):
        warn(self.tr("""{} seconds have passed. Please select a video now.""").format(settings.warn_time))
        self.cancel_timer()
    ...
    def set_choice(self, comparison):
        ...
        self.ui.left_file_label.setText(self.tr("Video {}").format(self.pseudonym_left))
        self.ui.right_file_label.setText(self.tr("Video {}").format(self.pseudonym_right))
        self.comparison_id = comparison['comparisonId']
        self.statusBar().showMessage(self.tr('Play the videos and select an option'))
    ...
    def play_left(self):
        ...
        self.play_video(self.left)
        
    def play_right(self):
        print "Playing video B!"
        self.play_video(self.right)
        
    def play_video(self, location):
        try:
            os.startfile(os.path.join(settings.file_folder, location))
        except WindowsError as e:
            #TODO: if file is not found then warn it may have been moved
            warn(self.tr("There was an error starting the video. Please try again."))
            print e
...
class SettingsPanel(QDialog):
    ...
    def select_file_folder(self):
        folder = QFileDialog.getExistingDirectory(self, self.tr('Select the folder where videos are stored'), os.getcwd())
        print "Selecting folder", folder
