...
class PwvRound(ndb.Model):
    ...
    def list_by_judge_id(cls, judge_id):
        ...
        query = cls.query().filter(cls.judges == judge_id,
                                   cls.file_list != None,
                                   cls.opened == True)
...
def new_round_page_OLD():
    ...
    users = UserAccount.all()
    file_lists = FileList.all()
    return render_template('admin_round.html', user=user, users=users, 
                           file_lists=file_lists,
                           change_user=change_user_link(),
                           to_client={'roundNames': round_names})
...
def edit_round(round_id):
    ...
    other_fields = {'name', 'fileList', 'max_views', 'max_comparisons', 'max_views_by_user', 'max_comparisons_by_user', 'min_time', 'warn_time'}
...
def add_file_list():
    ...
    # If the name is already in use then add a suffix
    number = 1
